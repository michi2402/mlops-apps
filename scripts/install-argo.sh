...
#
# Argo CD's built-in Gateway check is kept as is. It reports a Gateway Progressing until it is
# Programmed, which needs an address; the Envoy Service is ClusterIP (components/envoy-gateway/
# envoy-proxy.yaml), so every environment assigns one and the first wave cannot stall on a
# missing LoadBalancer (evidence/local-clean CLEAN-02). A copy of that check that earlier
# revisions of this script installed is removed, so re-running against such a cluster
# restores the upstream check as well.
